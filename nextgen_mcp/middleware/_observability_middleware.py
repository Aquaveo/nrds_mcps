...
Without this middleware, server logs are dominated by HTTP-level
noise (``OPTIONS``, ``POST``, ``GET``) and FastMCP's protocol-layer
``Processing request of type CallToolRequest`` lines that don't name
the tool. Diagnosing a slow turn requires cross-referencing multiple
